...
public class ServerThread extends Thread {
    ...
    void processPayload(Payload p) {
        switch (p.getPayloadType()) {
			...
			if (currentRoom != null) {
				...
				// Check for bold text trigger
				message = message.replaceAll("\\*(.*?)\\*", "<b>$1</b>");
				// Check for italic text trigger
				message = message.replaceAll("\\_(.*?)\\_", "<i>$1</i>");
				// Check for underline text trigger
				message = message.replaceAll("\\&(.*?)\\&", "<u>$1</u>");
				// Check for color text trigger
				message = message.replaceAll("\\#\\[(.*?)\\](.*?)\\#\\[\\/color\\]", "<font color=\"$1\">$2</font>");
				p.setMessage(message);
				currentRoom.sendMessage(this, p.getMessage());
				//hn224 5/7/2023 adding html tag and regex into the code
                } else {
    ...
    public boolean unblock(String user) {
        if (mutedUsers.contains(user)) {
